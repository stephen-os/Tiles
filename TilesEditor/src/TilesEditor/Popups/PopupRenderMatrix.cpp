...
namespace Tiles::Editor
{
    ...
    void PopupRenderMatrix::InitializeDialog()
    {
        if (Ctx().HasProject())
        {
            ...
            TILES_EDITOR_INFO("PopupRenderMatrix::InitializeDialog: Initialized dialog for project '{}'", projectName);
        }
    }
    ...
    void PopupRenderMatrix::ResetToDefaults()
    {
        ...
        m_ShowSuccessMessage = false;
        TILES_EDITOR_INFO("PopupRenderMatrix::ResetToDefaults: Reset to layer's current render groups");
    }
    ...
    void PopupRenderMatrix::ApplyRenderGroupChanges()
    {
        ...
        for (const auto& [layerIdx, renderGroup] : m_LayerToRenderGroup)
        {
            if (layerIdx < layerStack.GetLayerCount())
            {
                ...
                if (layer.GetRenderGroup() != newGroup)
                {
                    ...
                    hasChanges = true;
                    TILES_EDITOR_INFO("PopupRenderMatrix::ApplyRenderGroupChanges: Set layer '{}' to render group {}",
        ...
        {
            Ctx().GetProject()->MarkAsModified();
            TILES_EDITOR_INFO("PopupRenderMatrix::ApplyRenderGroupChanges: Applied render group changes to project");
        }
    }

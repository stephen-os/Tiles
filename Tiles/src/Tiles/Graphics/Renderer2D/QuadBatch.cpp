...
namespace Tiles
{
	void QuadBatch::Init(const std::vector<uint32_t>& quadIndices)
	{
		VAO = VertexArray::Create();
		VBO = VertexBuffer::Create(MaxVertices * sizeof(QuadVertex));
		VBO->SetLayout({
		...
		Base = new QuadVertex[MaxVertices];
	...
	bool QuadBatch::Upload(RendererState& state)
	{
	...
	void QuadBatch::Flush(RendererState& state)
	{
		if (IndexCount > 0)
		{
			Shader->Bind();

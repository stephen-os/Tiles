...
namespace Tiles
{
	...
	// Serializes the tile's flags, transform, tint, and atlas sub-region.
	...
	Tile Tile::FromJSON(const nlohmann::json& jsonTile)
	{
		...
		if (jsonTile.contains(JSON::Tile::AtlasIndex))
			tile.SetAtlasIndex(jsonTile[JSON::Tile::AtlasIndex].get<size_t>());
		...
		if (jsonTile.contains(JSON::Tile::TextureCoords))
		{
			...
			if (textureCoordsArray.size() >= 4)
				tile.SetTextureCoords(glm::vec4(textureCoordsArray[0], textureCoordsArray[1], textureCoordsArray[2], textureCoordsArray[3]));
		}

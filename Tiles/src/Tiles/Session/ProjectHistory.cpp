...
namespace Tiles
{
	// Serializes one entry (path, name, last-accessed as a Unix timestamp).
	...
	// Reconstructs one entry from JSON, keeping defaults for missing fields.
	...
	// Loads the persisted history on construction.
	ProjectHistory::ProjectHistory()
	{
		Load();
	}

	// Saves the history on destruction if it changed.
	ProjectHistory::~ProjectHistory()
	{
		if (m_HasChanges)
			Save();
	...
	// Records a project as most-recent, de-duplicating and trimming to the cap.
	void ProjectHistory::AddProject(const std::filesystem::path& filePath, const std::string& displayName)
	{
		...
		m_HasChanges = true;
		TILES_ENGINE_INFO("ProjectHistory::AddProject: Added '{}' to project history", displayName);
	}

	// Removes any entry with the given path.
	void ProjectHistory::RemoveProject(const std::filesystem::path& filePath)
	{
		auto it = std::find_if(m_History.begin(), m_History.end(),
			[&filePath](const ProjectHistoryEntry& entry) { return entry.filePath == filePath; });
	...
	void ProjectHistory::Clear()
	{
		if (!m_History.empty())
		{
			m_History.clear();
			m_HasChanges = true;
			TILES_ENGINE_INFO("ProjectHistory::Clear: Cleared all project history");
		}
	}

	// The path to the history JSON file in the platform's app-data directory,
	...
	void ProjectHistory::RemoveInvalidEntries()
	{
		auto it = std::remove_if(m_History.begin(), m_History.end(),
			[](const ProjectHistoryEntry& entry) { return !std::filesystem::exists(entry.filePath); });
		...
		if (it != m_History.end())
		{
			...
			m_HasChanges = true;
			TILES_ENGINE_INFO("ProjectHistory::RemoveInvalidEntries: Removed {} invalid entries", removedCount);
		}
	}

	// Loads the history from disk, skipping entries that fail to parse or no
	// longer exist.
		...
		try
		{
			...
			if (json.contains("projectHistory") && json["projectHistory"].is_array())
			{
				for (const auto& entryJson : json["projectHistory"])
				{
					try
					{
						m_History.push_back(ProjectHistoryEntry::FromJSON(entryJson));
					}
					catch (const std::exception& e)
					{
						TILES_ENGINE_INFO("ProjectHistory::Load: Failed to parse entry: {}", e.what());
					}
				}
			}

			RemoveInvalidEntries();
			TILES_ENGINE_INFO("ProjectHistory::Load: Loaded {} project history entries", m_History.size());
		}
		catch (const std::exception& e)
		{
			TILES_ENGINE_INFO("ProjectHistory::Load: Failed to load project history: {}", e.what());
			m_History.clear();
		}
	}

	// Writes the history to disk when dirty.
	void ProjectHistory::Save()
	{
		...
		try
		{
			...
			if (!file.is_open())
			{
				TILES_ENGINE_INFO("ProjectHistory::Save: Could not open file for writing");
				return;
			}

			file << json.dump(4);
			m_HasChanges = false;
			TILES_ENGINE_INFO("ProjectHistory::Save: Saved {} project history entries", m_History.size());
		}
		catch (const std::exception& e)
		{
			TILES_ENGINE_INFO("ProjectHistory::Save: Failed to save project history: {}", e.what());
		}
	}
}

...
namespace Tiles
{
	// One entry in the recent-projects list: where a project lives, its display
	// name, and when it was last opened.
	struct ProjectHistoryEntry
	{
		...
		[[nodiscard]] static ProjectHistoryEntry FromJSON(const nlohmann::json& json);
	};

	// Most-recently-used list of opened/saved projects, persisted to a JSON file
	// in the platform's app-data directory. Loads on construction and saves on
	// destruction when dirty.
	...
	public:
		...
		[[nodiscard]] bool IsEmpty() const { return m_History.empty(); }
		[[nodiscard]] const ProjectHistoryEntry& GetEntry(size_t index) const { return m_History[index]; }
